...
INPUT_DIR = os.path.join(BASE_DIR, "data", "raw_json_data")
OUTPUT_DIR = os.path.join(BASE_DIR, "data", "preprocessed_json_data")
LOG_FILE = os.path.join(BASE_DIR, "preprocessing.log")
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(LOG_FILE, mode='a'),  # ✅ Saves logs to 'preprocessing.log'
...
JSON_FILES = ["articles.json", "blogs.json", "pdf_data.json"]
...
def preprocess_json_files():
    ...
    for file_name in JSON_FILES:
        ...
        if not os.path.exists(input_file_path):
            logger.warning(f"⚠️ Skipping {file_name}, file not found.")
            continue
        ...
        try:
            ...
            logger.error(f"⚠️ Error loading JSON {file_name}: {e}", exc_info=True)
            continue
        ...
        try:
            ...
            logger.error(f"⚠️ Error saving cleaned data for {file_name}: {e}", exc_info=True)
