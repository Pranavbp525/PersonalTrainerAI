    ...
    if client:
        try:
            ...
            with client.pipeline() as pipe:
                ...
                pipe.execute()
            log.info(f"Stored chat history in Redis for session: {session_id}") # Log successful store
        except redis.exceptions.RedisError as e:
...
def get_chat_history(session_id: str) -> list:
    ...
    if client:
        try:
            ...
            log.info(f"Retrieved chat history from Redis for session: {session_id}") # Log successful retrieval
            return history
        except redis.exceptions.RedisError as e:
            log.error(f"Error getting chat history from Redis: {e}")
            return []  # Return empty list on error
    log.info(f"Redis client not available.  Falling back to PostgreSQL for session: {session_id}") # Log fallback
    return []