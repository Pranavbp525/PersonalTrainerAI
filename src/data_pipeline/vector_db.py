...
try:
    ...
    # Logger might not be ready yet, use print for initial status
    print("INFO: Successfully imported gcs_utils using relative path.")
except ImportError:
    try:
        from gcs_utils import read_json_from_gcs
        gcs_utils_available = True
        print("WARNING: Could not perform relative import of gcs_utils. Falling back to direct import.")
    except ImportError:
        print("ERROR: CRITICAL: Could not import gcs_utils. Cannot read from GCS.")
        def read_json_from_gcs(*args, **kwargs): return None
...
if os.path.exists(dotenv_path):
    ...
    logger.warning(f".env file not found at {dotenv_path}. Relying on env vars.")
...
PROCESSED_BUCKET = os.getenv("PROCESSED_BUCKET", "ragllm-454718-processed-data")
...
if not PINECONE_API_KEY: logger.warning("PINECONE_API_KEY not found!")
    ...
    for item_index, item in enumerate(data):
        ...
        try:
            ...
            for chunk_idx, chunk_text in enumerate(chunks):
                ...
                chunked_data.append({"id": chunk_id, "text": chunk_text, "metadata": {"original_source": original_gcs_source, "title": title, "url": url, "data_source_type": source_type, "chunk_index": chunk_idx, "text": chunk_text}})
...
def generate_embeddings_batched(chunked_data, model_name=EMBEDDING_MODEL_NAME, batch_size=EMBEDDING_BATCH_SIZE):
    ...
    # (Keep this function as previously corrected - manual batching loop)
    if not chunked_data: logger.warning("No data for embedding."); return [], None
    logger.info(f"Initializing embedding model: {model_name}")
    try: model = HuggingFaceEmbeddings(model_name=model_name, model_kwargs={'device': 'cpu'}); logger.info("Embedding model initialized.")
    ...
    try:
        for i in tqdm(range(0, total_items, batch_size), desc="Generating Embeddings", total=total_batches):
            batch_items = chunked_data[i:i + batch_size]; batch_texts = [item.get("text", "") for item in batch_items]
            texts_for_embedding = [text for text in batch_texts if isinstance(text, str) and text.strip()]
            if not texts_for_embedding: logger.warning(f"Batch {i//batch_size + 1} empty."); all_embeddings.extend([None] * len(batch_texts)); continue
    ...
    chunks_with_embeddings = []; failed_count = 0
    for i, chunk_item in enumerate(chunked_data):
        emb = all_embeddings[i]
        if isinstance(emb, list) and len(emb) == EMBEDDING_DIMENSION: chunk_item["embedding"] = emb; chunks_with_embeddings.append(chunk_item)
...
def store_in_pinecone(chunks_with_embeddings, pc_client: Pinecone, target_index_name: str):
    ...
    # (Keep this function as previously corrected - with index check fix)
    ...
    try:
        ...
        index_list_obj = pc_client.list_indexes()
        ...
        vectors_to_upsert = []; skipped_count = 0
        ...
        for chunk in chunks_with_embeddings:
            ...
            if isinstance(vec_id, str) and isinstance(emb, list) and len(emb) == EMBEDDING_DIMENSION and isinstance(meta, dict):
                cleaned_meta = {str(k): (str(v)[:10000] + "..." if isinstance(v, str) and len(v.encode('utf-8')) > 40000 else v) for k, v in meta.items() if isinstance(v, (str, bool, int, float, list))}
                for k, v in cleaned_meta.items():
                    if isinstance(v, list): cleaned_meta[k] = [str(item) for item in v if isinstance(item, str)][:100]
...
def run_chunk_embed_store_pipeline(**kwargs):
    ...
    Logs errors for Pinecone storage failure but does NOT raise an exception for it,
    allowing the Airflow task to succeed. Still raises exceptions for critical init/embedding failures.
    """
    ...
    # Validate GCS Utils
    if not gcs_utils_available: task_log.error("CRITICAL: GCS Utils failed to import."); raise RuntimeError("GCS Utils failed to import, cannot read data.")
    ...
    target_pinecone_index = INDEX_NAME
    ...
    if not all_chunks: task_log.warning("No chunks generated."); return # Success, nothing to process
    ...
    if embed_model is None or not chunks_with_embeds: task_log.error("Embedding failed."); raise RuntimeError("Embedding failed.") # Critical failure

    # Phase 3: Store in Pinecone
    task_log.info("--- Phase 3: Storing in Pinecone ---")
    store_start = time.time()
    storage_success = store_in_pinecone(chunks_with_embeds, pc, target_pinecone_index)
    task_log.info(f"--- Phase 3 Finished ({time.time() - store_start:.2f}s). Success reported by store_in_pinecone: {storage_success} ---")

    # --- !!! MODIFICATION START !!! ---
    # Check if storage succeeded, but LOG AN ERROR instead of raising one
    if not storage_success:
         task_log.error("!!! Pinecone storage process encountered errors during upsert. Task will NOT fail, but data may be incomplete in Pinecone. Check previous logs for details. !!!")
         # raise RuntimeError("Pinecone storage process failed.") # <--- REMOVED THIS LINE
    # --- !!! MODIFICATION END !!! ---

    # Phase 4: Example Query (Optional - Runs even if storage had errors, but might query old data)
    task_log.info("--- Phase 4: Running Example Query (Optional) ---")
    ...
    task_log.info(f"--- Pipeline finished in {time.time() - start_time:.2f} seconds (NOTE: Pinecone storage may have had errors) ---")
    # Function completion without exception means Airflow task success
