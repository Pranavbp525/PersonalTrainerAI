...
class AdvancedRAGEvaluator:
    ...
    def __init__(
        ...
        # Get tracking URI from environment, fallback to default only if necessary (or raise error)
        mlflow_tracking_uri = os.getenv("MLFLOW_TRACKING_URI", "http://localhost:5000")
        if mlflow_tracking_uri == "http://localhost:5000":
             logger.warning("MLFLOW_TRACKING_URI not set in environment, falling back to http://localhost:5000. This might not work in Docker.")
             # Optionally raise an error if tracking is mandatory:
        ...
        self.mlflow_tracker = MLflowRAGTracker(
            ...
            # artifact_location can also be configured if needed
        )
