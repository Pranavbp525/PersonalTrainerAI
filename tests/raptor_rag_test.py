...
@pytest.fixture
def mock_raptor_rag():
    """Fixture to create a mocked RaptorRAG instance."""
...
def test_initialize_raptor_rag(mock_raptor_rag):
    """Test initialization of RaptorRAG."""
...
def test_plan_retrieval(mock_raptor_rag):
    """Test the retrieval planning method."""
...
#     """Test the document retrieval method."""
#     raptor_rag, _, mock_embeddings, _, _ = mock_raptor_rag
#
#     mock_embeddings.embed_query.return_value = [0.1, 0.2, 0.3]
#     raptor_rag.index.query.return_value = MagicMock(matches=[
#         MagicMock(id="doc1", score=0.95, metadata={"text": "Sample text"}),
#         MagicMock(id="doc2", score=0.90, metadata={"text": "Another sample text"})
#     ])
#
...
def test_perform_multi_step_reasoning(mock_raptor_rag):
    """Test the multi-step reasoning method."""
    raptor_rag, _, _, _, mock_llm_chain = mock_raptor_rag
...
def test_synthesize_answer(mock_raptor_rag):
    """Test the answer synthesis method."""
    raptor_rag, _, _, _, mock_llm_chain = mock_raptor_rag
...
#     raptor_rag, _, _, _, mock_llm_chain = mock_raptor_rag
#
#     # Mock the LLMChain invoke method to return the expected sequence
#     mock_llm_chain.invoke.side_effect = [
...
#         {"text": "Step 3 reasoning"},
#         {"text": "Reflection on reasoning"},
#         {"text": "Synthesized answer"}  # Ensure this matches the expected final result
#     ]
#
#     # Call the method and assert the final answer
